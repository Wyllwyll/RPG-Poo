        ...
        switch (this.race) {
            case "Elf": {
                ...
                if (opponent.flying) {
                    ...
                    return `attaque avec Strengh * lvl et un bonus de 10% => ${damage}, ${opponent.setDamage(damage)}`;
                }
                else {
                    damage=Math.round(this.getStrength() * this.getLvl() * 0.9);
                    return `attaque avec Strengh * lvl et un malus de 10% => ${damage}, ${opponent.setDamage(damage)}`;
                }
            }
            case "Human": {
                if (opponent.flying) {
            ...
            case "Dwarf": {
                ...
                return `attaque avec Strengh * lvl=> ${damage}, ${opponent.setDamage(damage)}`;
            }
        }
    }
